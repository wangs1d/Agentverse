...
export function MerchantDashboard() {
  ...
  const dashboardData: DashboardData | null = useMemo(() => {
    ...
    return { product: selectedProduct, funnel: calculateProductTotalFunnel(selectedProduct) }
  }, [selectedProduct])

  const overall = dashboardData?.funnel.aggregate
    ...
    <main className="flex flex-1 flex-col gap-3 overflow-hidden p-3">
      <div className="panel-surface flex items-center gap-3 rounded-xl px-5 py-3">
        ...
        <div className="flex-1">
          <h1 className="text-sm font-semibold text-ink-50">商家后台 · 转化漏斗</h1>
          <p className="text-[11px] text-ink-200">所有数据基于产品×人格的<span className="text-tech-light">真实匹配分</span>计算</p>
        </div>
      ...
      {selectedPersonaId && dashboardData && (
        <SinglePersonaFunnel
          funnel={dashboardData.funnel.perPersona.find((f) => f.persona.id === selectedPersonaId)}
        />
      )}
    </main>
  )
  ...
  return (
    <div className={`rounded-lg border p-2.5 ${ring}`}>
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-0.5 rounded bg-accent-success/15 px-1 py-0.5 text-[9px] text-accent-success">
          <ArrowUpRight className="h-2.5 w-2.5" />
          {delta}
        </div>
      </div>
      <div className="mt-1 font-mono text-lg font-semibold text-ink-50">
    ...
    <div className="panel-surface flex flex-col overflow-hidden rounded-xl">
      <div className="flex items-center justify-between border-b border-white/[0.05] px-4 py-3">
        <div>
          ...
          <p className="mt-0.5 text-[10px] text-ink-300">从 Agent 看到 → 最终转化（5 阶段）</p>
        </div>
...
function PersonaDistributionPanel({
  ...
  const sorted = useMemo(() => [...perPersona].sort((a, b) => b.revenue - a.revenue), [perPersona])
    ...
    <div className="panel-surface flex flex-col overflow-hidden rounded-xl">
      <div className="flex items-center justify-between border-b border-white/[0.05] px-4 py-3">
        <div>
          ...
          <p className="mt-0.5 text-[10px] text-ink-300">点击看单人漏斗细节</p>
        </div>
          ...
          return (
            ...
            >
              <div className="mb-1 flex items-center gap-2">
                <div className={`h-1.5 w-1.5 shrink-0 rounded-full ${tierColor}`} />
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="truncate text-[9px] text-ink-300">{f.persona.archetype}</div>
                </div>
                <div className="text-right">
                  <div className="font-mono text-[10px] text-ink-50">
                    ¥{(f.revenue / 1000).toFixed(1)}k
                  </div>
                  <div className="font-mono text-[9px] text-tech-light">{f.stages[4].count}单</div>
                </div>
              </div>
    ...
    <div className="panel-surface flex flex-col overflow-hidden rounded-xl border-tech-light/30">
      <div className="flex items-center justify-between border-b border-white/[0.05] bg-tech/[0.04] px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-semibold text-ink-50">{funnel.persona.name} · 单人漏斗</div>
      ...
      <div className="grid grid-cols-1 gap-2 p-3 md:grid-cols-5">
        {funnel.stages.map((s, i) => {
          ...
          return (
            <div key={i} className={`rounded-lg border bg-gradient-to-br p-2.5 ${colorBg}`}>
              ...
              {i > 0 && (
                <div className="mt-0.5 flex items-center gap-0.5 text-[9px] text-ink-300">
                  <span>转化</span>
                  <span className={`font-mono ${textColor}`}>{(rate * 100).toFixed(1)}%</span>
                </div>
              )}
              <div className="mt-1 text-[9px] leading-tight text-ink-300">{s.description}</div>
            </div>
          )
        })}
      </div>
